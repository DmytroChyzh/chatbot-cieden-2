...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function loadEstimationData() {
        try {
            // Шлях до файлу даних
            const dataUrl = './data/estimations.json';
            console.log("Завантажуємо дані з:", dataUrl);
            ...
            console.error('Помилка при завантаженні даних для оцінки:', error);
    ...
    */
        ...
        try {
            ...
            if (queryType === 'project_idea') {
                ...
                try {
                    ...
                    console.log("Визначений тип проекту:", projectType, "Складність:", complexity);
                    
                    // Визначаємо додаткові фази
                    const additionalPhases = determineAdditionalPhases(projectDescription);
                    console.log("Визначені додаткові фази:", additionalPhases);
                    
                    // Створюємо структуру сторінок на основі типу проекту
                    const pages = createPagesFromProjectType(projectType, complexity);
                    console.log("Створено структуру сторінок:", pages);
                    
                    // Розраховуємо загальний час
                    let totalTime = 0;
                    pages.forEach(page => {
                        // Базовий час залежно від складності
                        let pageTime = 0;
                        ...
                        // Якщо це лендінг, збільшуємо час
                        if (page.type === 'landing') {
                            pageTime *= 2;
                        }
                        
                        // Множимо на кількість сторінок цього типу
                        page.totalTime = pageTime * page.count;
                        totalTime += page.totalTime;
                    });
                    
                    // Додатковий час для додаткових фаз
                    const phaseAdditions = [];
                    if (additionalPhases.includes('research')) {
                        const researchTime = totalTime * 0.2; // +20% на дослідження
                        phaseAdditions.push({ name: 'Дослідження', time: researchTime });
                        totalTime += researchTime;
                    }
                    if (additionalPhases.includes('prototyping')) {
                        const prototypeTime = totalTime * 0.15; // +15% на прототипування
                        phaseAdditions.push({ name: 'Прототипування', time: prototypeTime });
                        totalTime += prototypeTime;
                    }
                    if (additionalPhases.includes('testing')) {
                        const testingTime = totalTime * 0.1; // +10% на тестування
                        phaseAdditions.push({ name: 'Тестування', time: testingTime });
                    ...
                    // Зберігаємо результат оцінки для можливого експорту
                    lastEstimateResult = {
                        projectName: projectType,
                        pages: pages,
                        additionalPhases: phaseAdditions,
                        totalTime: totalTime,
                        totalCost: totalCost,
                        date: new Date().toISOString()
                    };
                    ...
                    const estimateMessage = generateEstimateMessage(pages, phaseAdditions, totalTime, totalCost);
                    
                    console.log("--- analyzeRequest END (project_idea) ---");
                    ...
                    return currentLanguage === 'ua' 
                        ? "Вибачте, виникла помилка при розрахунку вартості проекту. Будь ласка, спробуйте сформулювати запит інакше або зв'яжіться з нами напряму."
                        : "Sorry, there was an error calculating project cost. Please try rephrasing your request or contact us directly.";
                }
            }
            // Якщо це запит на оцінку проекту
            else if (queryType === 'estimation') {
                console.log("Обробка запиту на оцінку проекту");
                if (!estimationsData) {
                    console.log("Завантаження даних оцінювання...");
                    estimationsData = await loadEstimationData();
                }
                ...
                try {
                    ...
                    console.log("Визначена складність проекту:", projectComplexity);
                    ...
                    // Типові базові сторінки для оцінки
                    pages = [
                        { 
                            name: "Головна сторінка", 
                            complexity: projectComplexity,
                            quantity: 1,
                            min: projectComplexity === 'низька' ? 4 : projectComplexity === 'висока' ? 16 : 8, 
                            likely: projectComplexity === 'низька' ? 8 : projectComplexity === 'висока' ? 24 : 16, 
                            max: projectComplexity === 'низька' ? 16 : projectComplexity === 'висока' ? 40 : 24 
                        },
                        { 
                            name: "Типова контентна сторінка", 
                            complexity: projectComplexity,
                            quantity: 3,
                            min: projectComplexity === 'низька' ? 2 : projectComplexity === 'висока' ? 8 : 4, 
                            likely: projectComplexity === 'низька' ? 4 : projectComplexity === 'висока' ? 16 : 8, 
                            max: projectComplexity === 'низька' ? 8 : projectComplexity === 'висока' ? 24 : 16 
                        }
                    ];
                    
                    // Додаємо спеціальні сторінки залежно від типу проекту
                    if (messageLC.includes('інтернет-магазин') || messageLC.includes('e-commerce') || messageLC.includes('магазин')) {
                        pages.push({ 
                            name: "Каталог товарів", 
                            complexity: projectComplexity,
                            quantity: 1,
                            min: projectComplexity === 'низька' ? 6 : projectComplexity === 'висока' ? 16 : 10, 
                            likely: projectComplexity === 'низька' ? 10 : projectComplexity === 'висока' ? 24 : 16, 
                            max: projectComplexity === 'низька' ? 16 : projectComplexity === 'висока' ? 40 : 24 
                        });
                        pages.push({ 
                            name: "Сторінка товару", 
                            complexity: projectComplexity,
                            quantity: 1,
                            min: projectComplexity === 'низька' ? 4 : projectComplexity === 'висока' ? 12 : 8, 
                            likely: projectComplexity === 'низька' ? 8 : projectComplexity === 'висока' ? 20 : 14, 
                            max: projectComplexity === 'низька' ? 16 : projectComplexity === 'висока' ? 32 : 24 
                        });
                        pages.push({ 
                            name: "Корзина і оформлення", 
                            complexity: projectComplexity,
                            quantity: 1,
                            min: projectComplexity === 'низька' ? 8 : projectComplexity === 'висока' ? 20 : 14, 
                            likely: projectComplexity === 'низька' ? 16 : projectComplexity === 'висока' ? 32 : 24, 
                            max: projectComplexity === 'низька' ? 24 : projectComplexity === 'висока' ? 48 : 36 
                        });
                    } 
                    else if (messageLC.includes('лендінг') || messageLC.includes('landing')) {
                        // Заміняємо типові сторінки на лендінг
                        pages = [{ 
                            name: "Лендінг-сторінка", 
                            complexity: projectComplexity,
                            quantity: 1,
                            min: projectComplexity === 'низька' ? 8 : projectComplexity === 'висока' ? 24 : 16, 
                            likely: projectComplexity === 'низька' ? 16 : projectComplexity === 'висока' ? 40 : 24, 
                            max: projectComplexity === 'низька' ? 24 : projectComplexity === 'висока' ? 60 : 40 
                        }];
                    }
                    else if (messageLC.includes('портал') || messageLC.includes('portal')) {
                        pages.push({ 
                            name: "Сторінка статті", 
                            complexity: projectComplexity,
                            quantity: 1,
                            min: projectComplexity === 'низька' ? 4 : projectComplexity === 'висока' ? 12 : 8, 
                            likely: projectComplexity === 'низька' ? 8 : projectComplexity === 'висока' ? 20 : 14, 
                    ...
                    // Якщо є аналіз файлів, використовуємо цю інформацію
                    if (fileAnalysisResult && fileAnalysisResult.pageCount > 0) {
                        if (fileAnalysisResult.pages && fileAnalysisResult.pages.length > 0) {
                            pages = fileAnalysisResult.pages.map(page => {
                                // Для спрощення використовуємо середні значення за складністю
                                ...
                                const complexity = complexityValues[page.complexity] || complexityValues['середня'];
                    ...
                    // Розрахунок часу за методом PERT для кожної сторінки
                    const pertResults = pages.map(page => {
                        const pertTime = calculatePertEstimate(page.min, page.likely, page.max);
                        return {
                            ...page,
                            estimatedTime: Math.round(pertTime * 10) / 10, // Округлення до 1 десяткового знаку
                            totalTime: Math.round(pertTime * page.quantity * 10) / 10
                        };
                    });
                    
                    // Розрахунок загального часу на сторінки
                    let totalTime = pertResults.reduce((sum, page) => sum + page.totalTime, 0);
                    
                    // Додаткові фази
                    let additionalPhasesDetails = [];
                    
                    // Визначаємо необхідність кожної додаткової фази
                    let totalUniquePages = pertResults.reduce((sum, page) => sum + page.quantity, 0);
                    
                    // Дизайн система
                    if (additionalPhases.designSystem || totalUniquePages > 3) {
                        const dsTime = totalUniquePages < 5 ? 15 : totalUniquePages < 10 ? 24 : 40;
                        additionalPhasesDetails.push({
                            name: "Дизайн система",
                            description: "Розробка дизайн-системи: кольори, типографіка, компоненти, стилі, тощо",
                            time: dsTime,
                            min: 15,
                    ...
                    // Discovery фаза
                    if (additionalPhases.discovery) {
                        const discoveryTime = totalUniquePages < 5 ? 8 : totalUniquePages < 10 ? 16 : 32;
                        additionalPhasesDetails.push({
                            name: "Discovery фаза",
                            description: "Аналіз вимог, дослідження користувачів, конкурентний аналіз",
                            time: discoveryTime,
                            min: 5,
                            likely: 16,
                            max: 40
                    ...
                    // Wireframes (прототипи)
                    if (additionalPhases.wireframes) {
                        const wireframesTime = totalTime * 0.4; // 40% від загального часу на UI дизайн
                        additionalPhasesDetails.push({
                            name: "Wireframes/Прототипи",
                            description: "Створення прототипів та wireframes перед UI дизайном",
                            time: Math.round(wireframesTime * 10) / 10,
                            min: wireframesTime * 0.7,
                            likely: wireframesTime,
                            max: wireframesTime * 1.3
                        });
                        totalTime += wireframesTime;
                    }
                    
                    // Додаємо час на комунікацію (20%)
                    const communicationTime = totalTime * 0.2;
                    additionalPhasesDetails.push({
                        name: "Комунікація з клієнтом",
                        description: "Комунікація, презентації, уточнення вимог, погодження",
                        time: Math.round(communicationTime * 10) / 10,
                        min: communicationTime * 0.7,
                        likely: communicationTime,
                        max: communicationTime * 1.3
                    });
                    totalTime += communicationTime;
                    
                    // Розрахунок вартості (погодинна ставка $40)
                    const hourlyRate = 40;
                    const totalCost = totalTime * hourlyRate;
                    
                    // Зберігаємо результат оцінки для використання в експорті та історії
                    lastEstimateResult = {
                        projectName,
                        date: new Date().toISOString(),
                        pages: pertResults,
                        additionalPhases: additionalPhasesDetails,
                        totalTime,
                        totalCost
                    };
                    
                    // Оновлюємо графіки
                    updateCharts(pertResults, additionalPhasesDetails, totalTime, totalCost);
                    
                    // Генеруємо повідомлення з оцінкою
                    const estimateMessage = generateEstimateMessage(pertResults, additionalPhasesDetails, totalTime, totalCost);
                    
                    // Додатковий коментар про AI-аналіз файлів
                    let finalMessage = estimateMessage;
                    if (files.length > 0) {
                        finalMessage += `\n\nℹ️ *На основі аналізу завантажених файлів виявлено ${totalUniquePages} унікальних сторінок. Ці дані враховано в оцінці.*`;
                    } else {
                        finalMessage += "\n\n*Примітка: Це орієнтовна оцінка на основі наявної інформації. Для більш точної оцінки рекомендуємо надати детальнішу специфікацію проекту.*";
                    }
                    
                    console.log("Успішно оброблено запит оцінки проекту");
                    return {
            ...
            // Загальні запити - передаємо на обробку іншим функціям
            else {
                console.log("Загальний запит, передаємо на обробку getGeneralResponse");
                return await getGeneralResponse(queryType, userMessage);
            }
        } catch (error) {
            console.error("Загальна помилка в analyzeRequest:", error);
            console.error("Stack trace:", error.stack);
            return currentLanguage === 'ua'
    ...
    async function getGeneralResponse(queryType, message) {
        // Спочатку спробуємо використати AI для відповіді
        try {
            if (OPENAI_API_KEY && OPENAI_API_KEY !== 'your-api-key-here') {
                const aiResponse = await callOpenAI(message, false);
                return aiResponse;
            }
        } catch (error) {
            console.error('Помилка при використанні AI:', error);
            // Якщо сталася помилка, використовуємо резервний варіант зі шаблонами
        }
        
        // Отримуємо відповідний шаблон відповіді
        const templates = currentLanguage === 'ua' ? responseTemplates[queryType] : responseTemplates_en[queryType];
        
        // Якщо шаблон - функція, викликаємо її
        if (typeof templates === 'function') {
            return templates(message);
        }
        
        // Інакше вибираємо випадковий шаблон зі списку
    ...
    async function callOpenAI(userMessage, isEstimationRequest, customSystemPrompt) {
        try {
            console.log("Початок виклику OpenAI API...");
            
            // Перевіряємо наявність API ключа
            if (!OPENAI_API_KEY || OPENAI_API_KEY === 'your-api-key-here') {
                console.warn("API ключ OpenAI не налаштовано!");
                throw new Error("API ключ OpenAI не налаштовано. Будь ласка, налаштуйте API ключ у .env файлі серверу.");
            }
            ...
            // Підготовка системного промпту залежно від типу запиту, тільки якщо не передано кастомний промпт
            ...
            if (!AI_MODEL) {
                console.warn("Модель AI не налаштована, використовуємо gpt-3.5-turbo за замовчуванням");
            ...
            const requestBody = {
                ...
                messages: [
                    ...
                    ...recentHistory // Додаємо історію розмови
                ],
                temperature: 0.7,
                max_tokens: 1000
            };
            ...
            // Використовуємо відносний шлях до API замість повного URL
            ...
            if (!response.ok) {
                console.error("Помилка відповіді сервера:", response.status, response.statusText);
                throw new Error(`Помилка сервера: ${response.status} ${response.statusText}`);
            }
            
            // Отримуємо дані відповіді
            const data = await response.json();
            console.log("Отримана відповідь від OpenAI:", data);
            ...
            if (!data.choices || !data.choices[0] || !data.choices[0].message || !data.choices[0].message.content) {
                throw new Error("Отримана порожня відповідь від API");
            }
            
            const aiResponse = data.choices[0].message.content;
            console.log("Очищена відповідь від AI:", aiResponse.substring(0, 100) + "...");
            ...
            return `🔴 Виникла помилка при запиті до AI: ${error.message}\n\nСпробуйте ще раз або зверніться до адміністратора для перевірки API ключа.`;
        }
    }
    ...
    async function handleMessageSend() {
        console.log("=== Початок обробки повідомлення ===");
        const userMessage = messageInput.value.trim();
        if (userMessage === '') return;
        
        console.log("Повідомлення користувача:", userMessage);
        ...
        console.log("API ключ налаштовано:", hasApiKey);
        
        chatMessages.appendChild(thinkingElement);
        
        // Таймаут для імітації обробки
        await new Promise(resolve => setTimeout(resolve, 1500));
        
        try {
            // Вибираємо режим відповіді: використання API або тестові відповіді
            let botResponse;
            let isEstimationRelated = false;
            
            // Перевірка на кнопки пропозицій для швидкої відповіді
            if (userMessage.includes('Вартість дизайну') || 
                userMessage.includes('Розрахувати') || 
                userMessage.includes('Оцінити проект')) {
                
                console.log("Обробка запиту з кнопки пропозиції...");
                botResponse = getTestResponse(userMessage);
                isEstimationRelated = true;
            } 
            else {
                // Перевіряємо, чи запит стосується оцінки вартості
                isEstimationRelated = userMessage.toLowerCase().includes('вартість') || 
                                     userMessage.toLowerCase().includes('оцінит') || 
                                     userMessage.toLowerCase().includes('розрахуват') || 
                                     userMessage.toLowerCase().includes('дизайн');
                
                // Спроба отримати відповідь від AI
                if (hasApiKey) {
                    try {
                        console.log("Надсилаємо запит до OpenAI...");
                        botResponse = await callOpenAI(userMessage, isEstimationRelated);
                    } catch (aiError) {
                        console.error("Помилка при виклику OpenAI:", aiError);
                        // При помилці з API використовуємо тестову відповідь
                        botResponse = getTestResponse(userMessage);
                    }
                } else {
                    console.log("API ключ не налаштовано, використовуємо тестову відповідь");
                    botResponse = getTestResponse(userMessage);
                }
            }
            ...
            // Перевіряємо, чи результат є об'єктом з типом
            if (typeof botResponse === 'object' && botResponse.type) {
                console.log("Обробка структурованої відповіді типу:", botResponse.type);
                if (botResponse.type === 'complex_response' || botResponse.type === 'error') {
                    // Додаємо повідомлення бота
                    addMessage(botResponse.message);
                } else {
                    // Універсальна обробка для інших типів
                    addMessage(botResponse.message || JSON.stringify(botResponse));
                }
            } else {
                console.log("Обробка текстової відповіді");
                // Звичайний текстовий результат
            ...
            if (actionButtons) {
                actionButtons.style.display = isEstimationRelated ? 'flex' : 'none';
            }
            
            // Активуємо кнопки експорту тільки якщо є результати оцінки
            if (isEstimationRelated && lastEstimateResult) {
                // Активуємо кнопки експорту та історії
                if (exportPDFButton) exportPDFButton.disabled = false;
                if (exportExcelButton) exportExcelButton.disabled = false;
                if (saveEstimateButton) saveEstimateButton.disabled = false;
                if (showHistoryButton) showHistoryButton.disabled = false;
                if (showRiskAnalysisButton) {
                    showRiskAnalysisButton.disabled = false;
                    showRiskAnalysisButton.onclick = openChartsModal;
                }
            }
            
            console.log("=== Завершення обробки повідомлення ===");
        } catch (error) {
    ...
    clearButton.addEventListener('click', function() {
        ...
        conversationHistory = []; // Очищаємо історію розмови
    ...
    suggestionButtons.forEach(button => {
        button.addEventListener('click', function() {
            console.log("Натиснуто кнопку пропозиції:", button.textContent.trim());
            ...
            // додаємо "Розрахуйте", щоб чат краще розпізнав це як запит на оцінку
            if (buttonText.includes('дизайн') || 
                buttonText.includes('Дизайн') || 
                buttonText.includes('проект') || 
                buttonText.includes('Проект') || 
                buttonText.toLowerCase().includes('оцін') ||
                buttonText.includes('портал') ||
                buttonText.includes('e-commerce')) {
                
                messageInput.value = "Розрахуйте " + buttonText;
                console.log("Модифіковано текст запиту:", messageInput.value);
            }
    ...
    function determineQueryType(message) {
        console.log("Визначаємо тип запиту для:", message);
        
        // Визначаємо мову повідомлення
        currentLanguage = detectLanguage(message);
        console.log("Визначена мова:", currentLanguage);
        ...
        // Перевірка запитів на створення проектів
        ...
        if (isProjectIdeaRequest) {
            console.log("Визначено тип запиту: project_idea");
            return 'project_idea';
        }
        ...
        // Перевірка запитів на оцінку проектів
        const isEstimationRequest = estimationKeywords.some(keyword => 
            message.toLowerCase().includes(keyword.toLowerCase())
        );
        
        // Перевіряємо текст кнопок
        const buttonEstimationTexts_ua = [
            'вартість дизайну', 'розрахувати', 'оцінити проект', 'дизайн мобільного', 'дизайн інформаційного'
        ];
        
        const buttonEstimationTexts_en = [
            'calculate', 'estimate', 'mobile app', 'website design', 'e-commerce'
        ];
        
        const buttonTexts = currentLanguage === 'ua' ? buttonEstimationTexts_ua : buttonEstimationTexts_en;
        
        // Перевірка на текст кнопок
        const isButtonText = buttonTexts.some(text => 
            message.toLowerCase().includes(text.toLowerCase())
        );
        
        // Якщо це запит на оцінку проекту або текст з кнопки пропозиції
        if (isEstimationRequest || isButtonText) {
            console.log("Визначено тип запиту: estimation");
            return 'estimation';
        }
        ...
        for (const [category, patterns] of Object.entries(categories)) {
            for (const pattern of patterns) {
                if (pattern.test(message)) {
                    console.log("Визначено тип запиту:", category);
                    return category;
                }
            }
        }
        ...
        console.log("Тип запиту не визначено, використовуємо: unknown");
        return 'unknown';
    }

    // Додаємо функцію для базової тестової відповіді, яка не залежить від AI
    function getTestResponse(message) {
        console.log("Генеруємо тестову відповідь для:", message);
        
        // Визначаємо тип проекту на основі запиту
        let projectType = 'website'; // тип проекту за замовчуванням
        let projectName = 'Веб-сайт';
        let pagesToCreate = [];
        let projectComplexity = 'medium';
        
        // Визначаємо тип проекту на основі запиту
        if (message.toLowerCase().includes('лендінг')) {
            projectType = 'landing';
            projectName = 'Лендінг';
            pagesToCreate = [
                { name: 'Лендінг', type: 'landing', complexity: 'medium', quantity: 1, count: 1, totalTime: 20 }
            ];
        } 
        else if (message.toLowerCase().includes('e-commerce') || message.toLowerCase().includes('ecommerce') || 
                message.toLowerCase().includes('магазин') || message.toLowerCase().includes('комерс')) {
            projectType = 'ecommerce';
            projectName = 'E-commerce';
            pagesToCreate = [
                { name: 'Головна сторінка', type: 'landing', complexity: 'complex', quantity: 1, count: 1, totalTime: 24 },
                { name: 'Каталог товарів', type: 'catalog', complexity: 'complex', quantity: 1, count: 1, totalTime: 20 },
                { name: 'Сторінка товару', type: 'product', complexity: 'medium', quantity: 1, count: 1, totalTime: 16 },
                { name: 'Кошик', type: 'form', complexity: 'complex', quantity: 1, count: 1, totalTime: 12 },
                { name: 'Оформлення замовлення', type: 'form', complexity: 'complex', quantity: 1, count: 1, totalTime: 16 },
                { name: 'Особистий кабінет', type: 'dashboard', complexity: 'medium', quantity: 1, count: 1, totalTime: 20 },
                { name: 'Про компанію', type: 'content', complexity: 'simple', quantity: 1, count: 1, totalTime: 8 },
                { name: 'Контакти', type: 'form', complexity: 'simple', quantity: 1, count: 1, totalTime: 6 }
            ];
            projectComplexity = 'complex';
        }
        else if (message.toLowerCase().includes('інформаційн') || message.toLowerCase().includes('портал')) {
            projectType = 'portal';
            projectName = 'Інформаційний портал';
            pagesToCreate = [
                { name: 'Головна сторінка', type: 'landing', complexity: 'medium', quantity: 1, count: 1, totalTime: 16 },
                { name: 'Каталог статей', type: 'catalog', complexity: 'medium', quantity: 1, count: 1, totalTime: 12 },
                { name: 'Сторінка статті', type: 'content', complexity: 'medium', quantity: 1, count: 1, totalTime: 10 },
                { name: 'Сторінка категорії', type: 'catalog', complexity: 'medium', quantity: 1, count: 1, totalTime: 8 },
                { name: 'Про проект', type: 'content', complexity: 'simple', quantity: 1, count: 1, totalTime: 6 },
                { name: 'Контакти', type: 'form', complexity: 'simple', quantity: 1, count: 1, totalTime: 6 }
            ];
        }
        else if (message.toLowerCase().includes('мобільн') || message.toLowerCase().includes('додаток')) {
            projectType = 'mobile';
            projectName = 'Мобільний додаток';
            pagesToCreate = [
                { name: 'Вхід/Реєстрація', type: 'form', complexity: 'medium', quantity: 1, count: 1, totalTime: 12 },
                { name: 'Головний екран', type: 'dashboard', complexity: 'complex', quantity: 1, count: 1, totalTime: 24 },
                { name: 'Профіль користувача', type: 'profile', complexity: 'medium', quantity: 1, count: 1, totalTime: 16 },
                { name: 'Налаштування', type: 'settings', complexity: 'medium', quantity: 1, count: 1, totalTime: 12 },
                { name: 'Пошук', type: 'search', complexity: 'complex', quantity: 1, count: 1, totalTime: 20 },
                { name: 'Повідомлення', type: 'messages', complexity: 'complex', quantity: 1, count: 1, totalTime: 24 }
            ];
            projectComplexity = 'complex';
        }
        else {
            // За замовчуванням - стандартний веб-сайт
            pagesToCreate = [
                { name: 'Головна сторінка', type: 'landing', complexity: 'medium', quantity: 1, count: 1, totalTime: 12 },
                { name: 'Сторінка про компанію', type: 'content', complexity: 'simple', quantity: 1, count: 1, totalTime: 8 },
                { name: 'Сторінка послуг', type: 'content', complexity: 'medium', quantity: 1, count: 1, totalTime: 10 },
                { name: 'Сторінка контактів', type: 'form', complexity: 'simple', quantity: 1, count: 1, totalTime: 6 }
            ];
        }
        
        // Перевіряємо на ключові слова для різних типів відповідей
        if (message.toLowerCase().includes('дизайн') || message.toLowerCase().includes('вартість') || 
            message.toLowerCase().includes('оцінити') || message.toLowerCase().includes('розрахувати')) {
            
            // Додаткові фази залежно від типу проекту та складності
            const additionalPhases = [];
            
            // Дизайн система - для всіх, але різної складності
            if (projectComplexity === 'complex') {
                additionalPhases.push({ name: 'Дизайн система', time: 30 });
            } else if (projectType === 'landing') {
                additionalPhases.push({ name: 'Дизайн система', time: 10 });
            } else {
                additionalPhases.push({ name: 'Дизайн система', time: 15 });
            }
            
            // Wireframes/Прототипи
            const pagesTime = pagesToCreate.reduce((sum, page) => sum + page.totalTime, 0);
            const wireframesTime = Math.round(pagesTime * (projectType === 'mobile' ? 0.5 : 0.4));
            additionalPhases.push({ name: 'Wireframes/Прототипи', time: wireframesTime });
            
            // Дослідження - тільки для складніших проектів
            if (projectType === 'ecommerce' || projectType === 'portal' || projectType === 'mobile') {
                additionalPhases.push({ name: 'Дослідження користувачів', time: Math.round(pagesTime * 0.3) });
            }
            
            // Комунікація з клієнтом - для всіх проектів
            const communicationTime = Math.round(pagesTime * 0.2);
            additionalPhases.push({ name: 'Комунікація з клієнтом', time: communicationTime });
            
            // Розрахунок загального часу та вартості
            const baseTime = pagesToCreate.reduce((sum, page) => sum + page.totalTime, 0);
            const additionalTime = additionalPhases.reduce((sum, phase) => sum + phase.time, 0);
            const totalTime = baseTime + additionalTime;
            const hourlyRate = 40;
            const totalCost = totalTime * hourlyRate;
            
            // Оновлюємо графіки
            updateCharts(pagesToCreate, additionalPhases, totalTime, totalCost);
            ...
            // Форматуємо відповідь у вигляді таблиці
            const formattedResponse = `
## 📊 Оцінка вартості UI дизайну: ${projectName}

### Детальна оцінка
| Сторінка | Складність | Кількість | Час (год) |
|----------|------------|-----------|-----------|
${pagesToCreate.map(page => `| ${page.name} | ${page.complexity === 'simple' ? 'Проста' : page.complexity === 'medium' ? 'Середня' : 'Складна'} | ${page.quantity} | ${page.totalTime} |`).join('\n')}

### Додаткові фази
| Фаза | Час (год) |
|------|-----------|
${additionalPhases.map(phase => `| ${phase.name} | ${phase.time} |`).join('\n')}

### Загальна оцінка
⏱️ **Загальний час розробки:** ${totalTime} годин
💰 **Вартість проекту:** $${totalCost}

*Для перегляду графіків та детального аналізу натисніть кнопку "Показати графіки".*

*Примітка: Це орієнтовна оцінка на основі наявної інформації. Для більш точної оцінки рекомендуємо надати детальнішу специфікацію проекту.*
`;
            
            // Активуємо кнопку відображення графіків
            const showChartsButton = document.getElementById('showRiskAnalysis');
            if (showChartsButton) {
                showChartsButton.style.display = 'inline-block';
                showChartsButton.textContent = 'Показати графіки';
                
                // Оновлюємо обробник подій
                showChartsButton.onclick = openChartsModal;
            }
            
            return formattedResponse;
        }
        
        if (message.toLowerCase().includes('компані') || message.toLowerCase().includes('про вас')) {
            return `
Cieden - це компанія, що спеціалізується на UI/UX дизайні, заснована у 2015 році.
Наш головний офіс знаходиться у Києві, Україна, з додатковими офісами у США та ЄС.
У нашій команді працює більше 50 дизайнерів з досвідом у різних галузях.
Ми пропонуємо послуги UI/UX дизайну, дослідження користувачів, прототипування та брендингу.
            `;
        }
        
        if (message.toLowerCase().includes('привіт') || message.toLowerCase().includes('вітаю') || 
            message.toLowerCase().includes('добрий') || message.toLowerCase().includes('hello')) {
            return `Вітаю! Я чат-бот Cieden для оцінки вартості UI дизайну. Чим можу допомогти?`;
        }
        
        // Загальна відповідь для інших типів повідомлень
        return `
Дякую за ваше повідомлення. Я чат-бот Cieden, і я можу:

1. Оцінити вартість UI дизайну вашого проекту
2. Розповісти про компанію Cieden
3. Надати інформацію про наші послуги
4. Відповісти на загальні запитання

Будь ласка, уточніть, яка інформація вас цікавить.
        `;
    }

    // Функція для завантаження всіх доступних естімейтів
    async function loadAllEstimations() {
        try {
            const response = await fetch('/api/estimations');
            const data = await response.json();
            
            if (data.success && data.estimations) {
                console.log('Завантажено естімейти:', data.estimations);
                // Зберігаємо естімейти в глобальній змінній для подальшого використання
                window.availableEstimations = data.estimations;
                return data.estimations;
            } else {
                console.error('Помилка завантаження естімейтів:', data.error);
                return [];
            }
        } catch (error) {
            console.error('Помилка при завантаженні естімейтів:', error);
            return [];
        }
    }

    // Завантажуємо естімейти при старті
    document.addEventListener('DOMContentLoaded', async function() {
        await loadAllEstimations();
        // ... existing code ...
    });

    let conversationHistory = []; // Масив для зберігання історії повідомлень
});